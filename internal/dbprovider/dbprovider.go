...
//
// 原则：
//  1. 本工程所有持久化模块统一使用 MySQL/MariaDB（无 SQLite/JSONL/DuckDB/Redis 后端）；
//  2. 单库架构：全部模块共用统一的 GEO_MYSQL_DSN，不再保留各模块专属 *_MYSQL_DSN。
package dbprovider
...
// 具体实现位于 internal/dsnutil，以避免 config / dbprovider 之间的 import cycle。
func NormalizeMySQLDSN(raw string) string { return dsnutil.NormalizeMySQLDSN(raw) }
