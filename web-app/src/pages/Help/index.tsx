...
const Help: React.FC = () => {
  ...
  const loadArticleDetail = async (id: string) => {
    try {
      ...
      if (res) { setSelectedArticle(res); return }
    } catch {}
  ...
  const loadOnboarding = async () => {
    try {
      ...
      if (Array.isArray(steps)) {
        ...
        steps.forEach((s: any) => { state[s.step] = !!s.completed })
  ...
  const handleStepClick = (step: number, route: string) => {
    if (!onboardingState[step]) {
      ...
      api.help.completeStep(step).catch(() => {})
    }
    navigate(route)
  }
    ...
    <div className="help-page">
      ...
      <section className="help-hero">
        ...
        <div className="help-hero-content">
          <div className="help-hero-badge">📚 {t('help.title')}</div>
      ...
      <div className="help-content">
        ...
        <section className="help-section">
          <div className="help-section-header">
            <h2 className="help-section-title">{t('help.onboardingTitle')}</h2>
            <p className="help-section-subtitle">{t('help.onboardingSubtitle')}</p>
          </div>
          <div className="help-onboarding-grid">
            {onboardingSteps.map(item => {
              const completed = onboardingState[item.step] || false
              return (
                <div
                  key={item.step}
                ...
                >
                  <div className="help-onboarding-step">{completed ? '✓' : item.step}</div>
                  <div className="help-onboarding-icon">{item.icon}</div>
                  <div className="help-onboarding-title">{item.title}</div>
                  <div className="help-onboarding-desc">{item.desc}</div>
                  <span className={`help-onboarding-status ${completed ? 'is-done' : 'is-todo'}`}>
        ...
        <section className="help-section">
          <div className="help-section-header">
            <h2 className="help-section-title">{t('help.journeyTitle')}</h2>
            <p className="help-section-subtitle">{t('help.journeySubtitle')}</p>
          </div>
        ...
        <section className="help-section">
          <div className="help-section-header">
            <h2 className="help-section-title">{t('help.tabArticles')}</h2>
          </div>
          {selectedArticle ? (
