...
| 24 | 数据备份与恢复 | P1 | MySQL 每日定时 mysqldump + 异地存储，支持一键回滚；审计历史库定期冷备。 |
| 25 | 高可用与多副本 | P2 | API 无状态可多副本；MySQL 主从复制 + 只读副本，读请求分担。 |

## 四、数据层与 AI 能力升级

| # | 事项 | 优先级 | 说明 |
|---|------|--------|------|
| 40 | MySQL 性能调优与分区表 | P1 | 审计历史库月分区 / 离线工商库覆盖索引优化 / 慢查询治理；多用户并发写入与时序查询调优。 |
