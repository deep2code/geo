...
### Q: AI 判定结果错了怎么办？可以人工修正吗？

可以。自动判定（提及/引用/情感/位置）有异议时，运营人员可在审计报告页的「逐 Prompt 检测结果」表中点击 **✏️ 人工修正**：

- 修改「品牌提及 / 品牌引用 / 情感 / 提及位置」中的任意字段，并**必须填写修正原因**
- 系统**审计留痕**：记录修正人（JWT 用户邮箱）、修正时间、原因、**原值**（`results[].correction.prev_*`）
- 提交后**自动重算**聚合统计、BVS 评分、内容缺口、竞品声量、行动建议等全部指标，并原地更新历史记录（报告标注"已人工修正 N 条"）
- 修正接口：`POST /api/v1/admin/audit/correction`，权限要求 `PermManageData`（Owner/Admin；legacy API Key 模式全权）

> 同一查询×引擎的多次采样（`GEO_AUDIT_SAMPLES`）降低判定波动，人工修正是最终兜底——两者配合实现"判定可信 + 可纠错"。

### Q: 能研究"每个大模型喜欢采用哪里的文章"吗？

可以——**引擎来源偏好研究**（报告页「🧠 引擎来源研究」Tab）。每次品牌审计完成后，系统自动把每条引擎回答引用的 URL 提取为规范化域名（如 `zhihu.com`、`g2.com`），**append-only 记录**（`engine_source_citations` 表），随时间积累成历史：

- **各引擎 Top 来源**：ChatGPT / Kimi / GLM / DeepSeek… 各自引用最多的来源排行（次数、占比）
- **来源趋势**：某个来源（或整个引擎）的引用次数逐日变化，可点击域名下钻
